...
import { formatUserResponse } from './formatResponse.js';
...
const signup = async (req, res) => {
    try {
        ...
        res.status(201).json({ message: 'User registered successfully', user: formatUserResponse(user) });
    } catch (error) {
...
const updateUserProfile = async (req, res) => {
    try {
        ...
        await user.save();
        res.status(200).json({ message: 'Profile updated successfully', user: formatUserResponse(user) });
    } catch (error) {
